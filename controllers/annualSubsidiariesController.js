...
exports.getPublicResults = async (req, res) => {
    try {
        const query = "SELECT * FROM annual_subsidiaries_results WHERE status = 'Published' ORDER BY financial_year DESC";
...
exports.getAllResults = async (req, res) => {
    try {
        ...
        let query = 'SELECT * FROM annual_subsidiaries_results';
        let params = [];
        if (search) {
            query += ' WHERE financial_year LIKE ? OR document_type LIKE ?'; // document_type here is Company Name
...
exports.createResult = async (req, res) => {
    try {
        const { financialYear, documentType, status } = req.body; // documentType = Company Name
        ...
        const [result] = await db.query(
            'INSERT INTO annual_subsidiaries_results (financial_year, document_type, status, pdf_path) VALUES (?, ?, ?, ?)',
...
exports.updateResult = async (req, res) => {
    try {
        ...
        let query = 'UPDATE annual_subsidiaries_results SET financial_year=?, document_type=?, status=?';
...
exports.deleteResult = async (req, res) => {
    try {
        await db.query('DELETE FROM annual_subsidiaries_results WHERE id = ?', [req.params.id]);
